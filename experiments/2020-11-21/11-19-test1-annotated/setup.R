...
PARAM_VALS <- list(
  ...
  # Maximum colonization rate of forest: mean time of conversion
  # = 0.2, 0.1, 0.05, or 0.025 year if surrounded by maximally colonizing humans.
  max_rate_FH = c(5, 10, 20, 40),
  
  # Fraction of colonizations that come from anywhere on the lattice
  # (Need to explore a wider range of values)
  frac_global_FH = c(0, 0.01, 0.02, 0.1),
  
  # Rate of degradation of agriculture.
  # Corresponds to mean degradation time in years: 10, 5
  # Ratio (agricultural time) / (human abandonment time) = 0.5, 0.25
  max_rate_AD = c(0.1, 0.2),
  
  # How much the agricultural degradation rate is reduced with maximum
  # forest levels: by a factor of 20 (0.05) or by a factor of 4 (0.25).
  min_rate_frac_AD = c(0.05, 0.25),
