...
plot_beta_SS <- function(df) {
  ...
  pD <-df%>%
    ...
    facet_grid(rows = vars(max_rate_FH), cols = vars(rate_DF), labeller = labeller(.rows = label_both, .cols = label_both))+
    ggtitle("Degraded")
  ggsave(file.path("experiments/2020-12-07/e1-colonization-vbeta/", 'vbeta_states.png'),   gridExtra::grid.arrange(pH,pA,pF,pD), width = 12, height = 12)
  ...
  ggsave(file.path("experiments/2020-12-07/e1-colonization-vbeta/", 'vbeta_mean_sd_vs_rate_DF.png'),   beta_S, width = 8.5, height = 4,units = "in")
  ...
  ggsave(file.path("experiments/2020-12-07/e1-colonization-vbeta/", 'vbeta_mean_sd_vs_rate_DF_points.png'),   beta_points, width = 8.5, height = 5,units = "in")
...
plot_beta_states<-function(df){
  ...
  scale_size(range=c(0.1,2),guide="none")+
    ...
    ggsave(file.path("experiments/2020-12-07/e1-colonization-vbeta/", 'states_SS.png'),   gridExtra::grid.arrange(pH,pFD,ncol=1), width = 8.5, height = 5,units = "in")
}
