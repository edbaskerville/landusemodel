...
function get_rate_local_FH(s::Simulation)
    ...
    (1.0 - p.frac_global_FH) * state_count(s, H) * p.max_rate_FH
end
...
function do_event_local_FH!(s::Simulation, t)
    ...
    # Draw a random inhabited site to do the colonizing
    ...
    # If neighbor is in state F, maybe transition to state H
    ...
    if get_state(s, loc_neighbor) == F
        ...
        if rand(rng) < probability_FH(s, loc_H)
            ...
            set_state!(s, loc_neighbor, H, TBetaPair(s.t, beta))
            true
        else
            # @debug "not doing F -> H"
            false
...
function get_rate_FA(s)
    return state_count(s, H) * s.max_beta
end
...
function do_event_FA!(s, t)
    ...
    # If neighbor is in state F, maybe transition to state A
    # @debug "neighbor state" get_state(s, loc_neighbor)
    if get_state(s, loc_neighbor) == F
        # @debug "neighbor is F"

        # Observe beta at current time
