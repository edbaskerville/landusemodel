...
main <- function() {
  ...
  output_runs %>%
    dplyr::mutate(Variant = substring(productivity_function_FH, 4))%>%
    dplyr::mutate(`forest regeneration` = 1/rate_DF)%>%
    dplyr::filter(max_rate_FH ==40, time==1200,H>0)%>%
...
load_table <- function(tbl_name) {
  db <- dbConnect(SQLite(), 'experiments/2020-12-09/e1-colonization-vbeta/db.sqlite')
...
load_table_cb <- function(tbl_name) {
  db <- dbConnect(SQLite(), 'experiments/2020-12-09/e1-colonization-cbeta/db.sqlite')
  tbl <- dbGetQuery(db, str_glue('SELECT * FROM {tbl_name}'))
  dbDisconnect(db)
...
load_table_e3 <- function(tbl_name) {
  db <- dbConnect(SQLite(), 'experiments/2020-12-09/e3-global-vbeta/db.sqlite')
  tbl <- dbGetQuery(db, str_glue('SELECT * FROM {tbl_name}'))
  dbDisconnect(db)
...
plot_timeseries <- function(subdf) {
  p <- ggplot(subdf, aes(x = time, y = density, color = state)) +
    geom_line(size=1.2) +
    scale_color_manual(values = c("yellow","brown","darkgreen","#7ea6e0"),guide="none")+
    facet_grid(rows = vars(Variant), cols = vars(`forest regeneration`), labeller = labeller(.rows = label_both, .cols = label_both))+
  theme_minimal(base_size = 10)
  ggsave(file.path('experiments/2020-12-09/e1_timeseries_states_vb.png'), p, width = 8.5, height = 4,units = "in")
}
...
plot_beta <- function(subdf2) {
  p <- subdf2%>% 
    ...
    facet_grid(rows = vars(Variant), cols = vars(`forest regeneration`), labeller = labeller(.rows = label_both, .cols = label_both))+
    theme_minimal(base_size = 10)
  ggsave(file.path('experiments/2020-12-09/e1_beta_ts_vb.png'), p, width = 8.5, height = 4,units = "in")
}


plot_lifespan<-function(subdf3){

  ggsave(file.path('experiments/2020-12-09/e3_lifetime_land.png'), 
  
gridExtra::grid.arrange(  
  subdf3%>%
    ggplot(aes(x=`forest regeneration`,y=H_lifetime_avg,color=Variant))+geom_point()+geom_line()+scale_y_continuous("years",limits = c(0,35))+ggtitle("Human settlements")+scale_x_continuous("",breaks = c(5,10,25,50,100))+scale_color_manual(values=c('#999999','#E69F00'),guide="none")+theme_classic(base_size = 12)
,  
  
  subdf3%>%
    ggplot(aes(x=`forest regeneration`,y=A_lifetime_avg,color=Variant))+geom_point()+geom_line()+scale_y_continuous("",limits = c(0,8))+ggtitle("Agriculture")+scale_x_continuous("",breaks = c(5,10,25,50,100))+scale_color_manual(values=c('#999999','#E69F00'),guide="none")+theme_classic(base_size = 12)
,  
  subdf3%>%
    ggplot(aes(x=`forest regeneration`,y=F_lifetime_avg,color=Variant))+geom_point()+geom_line()+scale_y_continuous("years",limits = c(0,100))+ggtitle("Forest")+scale_x_continuous("Forest regeneration [year]",breaks = c(5,10,25,50,100))+scale_color_manual(values=c('#999999','#E69F00'),guide="none")+theme_classic(base_size = 12)
,  

  subdf3%>%
    ggplot(aes(x=`forest regeneration`,y=D_lifetime_avg,color=Variant))+geom_point()+geom_line()+scale_y_continuous("",limits = c(0,100))+ggtitle("Degraded")  +scale_x_continuous("Forest regeneration [year]",breaks = c(5,10,25,50,100)) +scale_color_manual(values=c('#999999','#E69F00'),guide="none")+theme_classic(base_size = 12)
,
ncol=2
),
...
#############################
  ggsave(file.path('experiments/2020-12-09/e3_land_frac_noguide.png'), 
...
plot_betastate_v_vs_c_beta<-function(df_v, df_c){
  ...
  ggsave(file.path('experiments/2020-12-09/e1_cb_vb_states.png'), 
