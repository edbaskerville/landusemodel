...
output_last <- (function() {
  ...
  df <- dbGetQuery(db_conn,
    'SELECT run_id, H, A, F, D, beta_mean FROM output WHERE time = 150.0'
  ) %>%
...
summary <- output_last %>%
  ...
  summarize(H = mean(H), A = mean(A), F = mean(F), D = mean(D), beta_mean = mean(beta_mean))
```
